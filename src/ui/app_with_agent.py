...
# =========================
# 1. Replaced with real LangGraph 
# =========================
def run_agent(query: str) -> Dict[str, Any]:
    """Execute real LangGraph agent pipeline."""
    try:
        # Import the compiled LangGraph agent
        from src.graph.graph import agent, ConversationState
        ...
        # Extract the spoken answer from final_answer
        final_answer = result.get("final_answer", {})
        spoken_summary = final_answer.get("spoken_summary", "")
        
        # If no spoken summary, create one from available data
        if not spoken_summary:
            product_count = len(result.get("reconciled_results", []))
            if product_count > 0:
                spoken_summary = f"I found {product_count} products matching your query."
            else:
                spoken_summary = f"I've analyzed your query about {query}."
        
        # Format products for display
        products = []
        reconciled = result.get("reconciled_results", [])
        
        for i, item in enumerate(reconciled[:3]):  # Top 3 products
            if isinstance(item, dict):
                products.append({
                    "sku": item.get("sku", f"PROD-{i+1:03d}"),
                    "title": item.get("title", item.get("name", "Unnamed Product")),
                    "brand": item.get("brand", "Unknown"),
                    "price": item.get("price", 0.0),
                    "rating": item.get("rating", 0.0),
                    "doc_id": item.get("doc_id", f"doc-{i+1}"),
                    "source_url": item.get("url", item.get("source_url", "#")),
                })
        
        # Create step log from node execution
        steps = []
        node_logs = result.get("node_logs", [])
        
        # Map node logs to steps
        for log in node_logs:
            if "[" in log and "]" in log:
                node_name = log.split("[")[1].split("]")[0].lower()
                steps.append({
                    "node": node_name,
                    "summary": log.split("] ")[-1] if "] " in log else log
                })
        
        # If no steps were extracted, create default ones
        if not steps:
            steps = [
                {"node": "router", "summary": "Analyzed query intent and constraints"},
                {"node": "planner", "summary": "Created search strategy for finding relevant products"},
                {"node": "retriever", "summary": "Retrieved and ranked products from knowledge base"},
                {"node": "answerer", "summary": "Generated detailed answer with product recommendations"}
            ]
        ...
        st.error(f"❌ Agent module import error: {e}")
...
def app() -> None:
   ...
   # Check if agent is available
    try:
        ...
        st.warning(f"⚠️ Agent not available: {e}. Using mock data.")
